...
<head>
  ...
  <title>Itutor | Manage categories</title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      <div class="col-md-12 col-sm-12 ">
        ...
        <div class="col-md-7">
       <!--  <form method="post" class="form-inline" action="<?php //echo base_url(); ?>Books/updateclasspriority"></form> -->
          <form method = "post" class="form-inline" action="<?php echo base_url(); ?>Books/manage_categories" >
           <input type="search" name="search" class="form-control" placeholder="Category title" value="<?php echo set_value('search'); ?>">
           <input type="submit" class="btn btn-md btn-info" value="Search" id="search_btn">
           </form>
           <form method="post" class="form-inline" action="<?php echo base_url(); ?>Books/updateclasspriority">
        </div>
        <div class="col-md-5 form-inline">
          <button  class="btn btn-success btn-md" name="active" > Active </button>
          <!--onclick="updateActivationStatus('1','categories')"-->
          <button  class="btn btn-warning btn-md"  name="inactive">In  Active </button>   
          <!--onclick="updateActivationStatus('1','categories')"-->                 
           <span onclick="return confirm('Confirm to delete')"><button name="delete"  class="btn btn-danger  btn-md" onclick="common_Delete('categories');"> Delete </button></span>
           <span><a  class="btn btn-primary  btn-md" href="<?php echo base_url(); ?>books/manage_categories" title="Refresh"><i class="fa fa-refresh"></i></a></span>
           <div class="col-md-1">
           <a href="<?php echo base_url(); ?>books/add_category" class="btn btn-primary pull-right">Add New</a></div>
        </div>
            ...
            <div class="box-body">
              ...
              <div class="col-sm-12">
                ...
                <?php if($this->session->flashdata('success')) { ?>
                              ...
                              <form method="post" action="<?php echo base_url();?>Books/updateclasspriority" id="priority">
              <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <tbody>
                          <?php
                            ...
                            $i = $segment+1;
                            $newresult = json_decode($categories);
                            if ($newresult->code == SUCCESS_CODE) {
                                foreach ($newresult->common_result as $row) {
                                    ?>
                                    <tr class="">
                                        <td><input type="checkbox" class="checkbox" name="multiple[]" value="<?php echo $row->category_id;?>"> </td>
                                        <td><?php echo $i; ?> </td>
                                        <td><?php echo $row->title; ?></td>
                                        <td><?php if($row->category_status){?><img src="<?php echo base_url(); ?>uploads/books/categories/<?php echo $row->image; ?>" style="width: 40px;height: 40px;"><?php }?></td>
                                ...
                                <tr><td colspan="10" style="text-align:center; color:red;font-size:20px;">No categories Found...</td></tr>
                            <?php }
                            ?>
      ...
      // alert(listarray);
        ...
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'rl_users_tbl', 'updatelist': checklist, 'activity': s},
                url: '<?php base_url()?>/books/commonStatus',
                success: function (u) {
                    console.log(u);
