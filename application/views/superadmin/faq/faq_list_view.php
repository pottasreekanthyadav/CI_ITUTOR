      ...
      <div class="col-md-12 col-sm-12 ">
        ...
        <div class="col-md-5 form-inline">
          ...
          <button  class="btn btn-success   btn-md" onclick="updateActivationStatus('1')"> Active </button>
           ...
           <div class="col-md-1">
           <a href="<?php echo SUPER_ADMIN_FOLDER_PATH; ?>Faqs/createFaq" class="btn btn-primary pull-right">Add New</a></div>
        </div>
                          ...
                          <?php
                            ...
                            if ($result->code == SUCCESS_CODE) {
                                foreach ($result->common_result as $row) {
                                    ?>
                                    <tr class="">
                                        <td><input type="checkbox" class="checkbox" name="multiple[]" value="<?php echo $row->faq_id;?>"> </td>
                                        <td><?php echo $i; ?> </td>
                                        <td><?php echo $row->faq_ref_no; ?></td>
...
?>
                                      <td> <?php echo $string; ?><?php if(strlen($string)>50){ ?><a  data-toggle="modal" data-target="#myModal12" href="#" onclick="populate_more('<?php echo $row->faq_id; ?>')" class="text-primary bb"><b>Read More</b></a><?php } ?></td>
                                        <p style="display:none" id='mydescreption<?php echo $row->faq_id; ?>'><?php echo $row->description; ?></p>
                                        <td class="text-success"><b><?php if ($row->reply_status == 1){
                                                    echo "<b style='color:green'>Active</b>";
                                                } elseif($row->reply_status == 0) {
                                                    echo "<b style='color:red'>Inactive</b>";
                                                } ?>   </b></td>
                                        <td> 
                                            <a href="<?php echo base_url(); ?>superadmin/Faqs/getFaqs/<?php echo $row->faq_id; ?>" class="btn btn-sm btn-warning "> Update <i class="glyphicon glyphicon-upload"> </i> </a><br/><br/>
                                        </td>
                                    </tr> 
                                    <?php $i++;
                                }
                               ...
                               <div class="modal-dialog modal-lg">
                                   <div class="modal-content">
                                       <div class="modal-header">
                                           ...
                                           <h4 class="modal-title" id="myModalLabel">Details Faq</h4>
                                       </div>
      ...
      // alert(listarray);
        ...
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'faq', 'updatelist': checklist, 'activity': s},
                url: '<?php echo base_url(); ?>superadmin/Faqs/commonStatus/',
                success: function (u) {
                    console.log(u);
    ...
    var listarray=new Array();
      ...
      if(checklist!=''){
         ...
         $.ajax({
          ...
          url:'<?php echo base_url();?>superadmin/Faqs/commonDelete/',
          success:function(u){
           // console.log(u);
            if(u.code=='200'){$('#success').show();$('#successmessage').html(u.description).addClass('alert alert-success');setTimeout(function() {window.location=location.href;},2000);}
                    if(u.code=='204' || u.code=='301' || u.code=='422'){$('#fail').show();$('#failmessage').html(u.description).addClass('alert alert-danger');}
