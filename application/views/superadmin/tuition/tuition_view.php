  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Tuition Center </li>
      </ol>
    </section>
                        ...
                        foreach($support_recs as $row)
                        {
                          ?>
                                    <tr class="">
                                        ...
                                        value="<?php echo $row->center_id;?>" </td>
                                        <td><?php echo $i; ?> </td>
                                  ...
                                  ?>
                                  <p style="display:none" id='mydescreption<?php echo $row->center_id; ?>'><?php echo $append_string;?></p>
                                  <?php
...
?>
                            ...
                            <td class="footable-visible"><?php echo $string; ?><?php if(strlen($string)>50){ ?><a  data-toggle="modal" data-target="#myModal12" href="#" onclick="populate_more('<?php echo $row->center_id; ?>')" class="text-primary bb"><b>Read More</b></a><?php } ?></td>
      ...
      // alert(listarray);
        ...
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'tuition', 'updatelist': checklist, 'activity': s},
                url: '<?php echo base_url();?>superadmin/tuition/common_status',
                success: function (u) {
                    console.log(u);
    ...
    var listarray=new Array();
      ...
      if(checklist!=''){
         ...
         $.ajax({
          dataType:'json',
          type:'post',
          data:{'tablename':'tuition','updatelist':checklist},
          url:'<?php echo base_url();?>superadmin/tuition/commonDelete',
          success:function(u){
           // console.log(u);
            if(u.code=='200'){$('#success').show();$('#successmessage').html(u.description).addClass('alert alert-success');setTimeout(function() {window.location=location.href;},2000);}
                    if(u.code=='204' || u.code=='301' || u.code=='422'){$('#fail').show();$('#failmessage').html(u.description).addClass('alert alert-danger');}
