  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Manage Tutors</li>
      </ol>
    </section>
    ...
    <section class="content">
     ...
     <button  class="btn btn-success   btn-md" onclick="updateActivationStatus('1')"> Active </button>
       ...
       <button  class="btn btn-success   btn-md" onclick="updateStatus('1')"> Verify</button>
           <!--  <button  class="btn btn-warning   btn-md"  onclick="updateActivationStatus('0')">In  Active </button>  -->  
          ...
          <div id="success"></div>
                  ...
                  $page=$this->uri->segment(4);
                        ...
                        foreach($tutor_recs as $row)
                        {
                          ?>
                                    <tr class="">
                                        <td><input type="checkbox" class="checkbox" name="multiple[]" value="<?php echo $row->user_id;?>"> </td>
                                        <td><?php echo $i;?>
                                        <td><?php echo $row->email; ?></td>
                                        <td><?php echo $row->mobile; ?></td>
                                        <td><?php echo $row->syls;?></td>
                                        <td><?php echo $row->subjects;?></td>
                                        <td><?php echo $row->cats;?></td>
                                        <td><?php echo ucfirst($row->gender);?></td>
                                        <td><?php echo ucwords($row->location); ?></td>
                                       <td><a href="<?php echo base_url(); ?>uploads/resume/<?php echo $row->resume; ?>" download="resume">Download</a></td>
                                       <td>
                                        <?php
                                        $adhaar=$row->adhaar_proof_name;
                                       $arr=explode(",",$adhaar);
                                       $count=count($arr);
                                       for($j=0;$j<$count;$j++)
                                       {
                                        ...
                                        ?>
                                        <a href="<?php echo base_url(); ?>uploads/adhaar/<?php echo $arr[$j]; ?>" download="adhaar">Download<?php echo $j+1;?></a>
                                      <?php } ?>
                                    </td>
                                        <td class="text-success"><b> <?php if ($row->tutor_status == 1) {
                                          ...
                                          <td class="text-success"><b> <?php if ($row->payment_status == 1) {
                                          ...
                                          <td><?php if($row->verification_status==1){?>
                             ...
                             }else  { ?> <tr><td colspan="10" style="text-align:center; color:red;font-size:20px;">Tutor Not Found...</td></tr>
                             <?php }
                             ?> 

                        </tbody>
                        <button class="btn-btn info"><a href="<?php echo base_url();?>/superadmin/Tutor/generate_csv_report" style="color:green">download csv file</a></button>
                    </table>
                         <div class="text-center">
              <td colspan="4" > <p class="pages"><?php if(!empty($tutor_links))echo $tutor_links; ?></p></td>
              </div>
                    </div></div></div>
            </div>
            <!-- /.box-body -->
      ...
      // alert(listarray);
        ...
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'tutor', 'updatelist': checklist, 'activity': s},
                url: '<?php echo base_url(); ?>superadmin/Tutor/commonStatus/',
                success: function (u) {
                    console.log(u);
      ...
      // alert(listarray);
        ...
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'verification', 'updatelist': checklist, 'activity': s},
                url: '<?php echo base_url(); ?>superadmin/Tutor/commonStatus/',
                success: function (u) {
                    console.log(u);
                    if(u.code=='200'){$('#success').show();$('#success').html(u.description).addClass('alert alert-success');setTimeout(function() {window.location=location.href;},2000);}
    ...
    var listarray=new Array();
      ...
      if(checklist!=''){
         ...
         $.ajax({
          dataType:'json',
          type:'post',
          data:{'tablename':'tutor','updatelist':checklist},
          url:'<?php echo base_url();?>superadmin/Tutor/commonDelete/',
          success:function(u){
           // console.log(u);
            if(u.code=='200'){$('#success').show();$('#success').html(u.description).addClass('alert alert-success');setTimeout(function() {window.location=location.href;},20);}
                    if(u.code=='204' || u.code=='301' || u.code=='422'){$('#fail').show();$('#fail').html(u.description).addClass('alert alert-danger');}
                 },
          error:function(er){
            console.log(er);
          }
