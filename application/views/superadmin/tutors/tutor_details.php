...
<?php $result=json_decode($common_result); //print_r($result);?>
    <!-- Main content -->
    <section class="content">
      ...
      <div class="col-md-12 col-sm-12 ">
        ...
        <div class="col-md-1">
           <a href="<?php echo SUPER_ADMIN_FOLDER_PATH;?>Subscription/addSubscription" class="btn btn-primary pull-right">Add New</a>
        </div>
      </div>
                        ...
                        <tbody>
                            ...
                            if ($standard_res->code == 200) {
                                foreach ($standard_res->common_result as $row) {
                                    ?>
                                    <tr class="">
                                        <td><input type="checkbox"  name="multiple[]" value="<?php echo $row->subscription_id; ?>">  </td>
                                        <td><?php echo $i; ?> </td>
                                        <td><?php echo $row->category_name;?></td>
                                        <td><?php echo $row->amount; ?></td>
                                        <td><?php echo $row->validity.' days'; ?></td>
										                    <td><?php echo $row->no_of_tutions; ?></td>
                                        ...
                                        <td> 
                                        <a href="<?php echo base_url(); ?>superadmin/Subscription/update_subscription_ctrl1/<?php echo $row->subscription_id; ?>" class="btn btn-sm btn-warning ">  <i class="glyphicon glyphicon-upload"> </i>Update </a><br/><br/>
                                        </td>
                                    </tr> 
                                    <?php $i++;
                                }
      ...
      //alert(checklist);
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'subscription', 'updatelist': checklist, 'activity': s},
                url: '<?php echo base_url(); ?>superadmin/Subscription/commonStatus/',
                success: function (u) {
                    console.log(u);
    ...
    var listarray=new Array();
      ...
      if(checklist!=''){
         ...
         $.ajax({
          dataType:'json',
          type:'post',
          data:{'tablename':'subscription','updatelist':checklist},
          url:'<?php echo base_url();?>superadmin/Subscription/commonDelete/',
          success:function(u){
           // console.log(u);
            if(u.code=='200'){$('#success').show();$('#successmessage').html(u.description).addClass('alert alert-success');setTimeout(function() {window.location=location.href;},2000);}
                    if(u.code=='204' || u.code=='301' || u.code=='422'){$('#fail').show();$('#failmessage').html(u.description).addClass('alert alert-danger');}
