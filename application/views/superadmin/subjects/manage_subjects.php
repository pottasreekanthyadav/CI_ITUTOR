      ...
      <div class="row">
        ...
        <div class="col-md-5 form-inline">
          ...
          <button  class="btn btn-success   btn-md" onclick="updateActivationStatus('1')"> Active </button>
           ...
           <div class="col-md-1">
           <a href="<?php echo SUPER_ADMIN_FOLDER_PATH; ?>Subjects/addSubjectView" class="btn btn-primary pull-right">Add New</a></div>
        </div>
            ...
            <div class="box-body">
              ...
              <div class="col-sm-12">
                ...
                <?php if($this->session->flashdata('success')) { ?>
                                ...
                                <?php } ?>
                <div id="success"></div>
                <form method="post" action="<?php echo base_url();?>superadmin/Subjects/updatesubjectpriority" id="priority">
                          ...
                          <?php
                            ...
                            if ($result->code == SUCCESS_CODE) {
                                foreach ($result->common_result as $row) {
                                    ?>
                                    <tr class="">
                                        <td><input type="checkbox" class="checkbox" name="multiple[]" value="<?php echo $row->subject_id;?>"> </td>
                                        <td><?php echo $i; ?> </td>
                                        <td><?php echo $row->subject_name; ?></td>
                                        <td><?php $added_date=$row->added_on; echo date("d-M-Y", strtotime($added_date)); ?></td>
                                        <td><?php $updated_on=$row->updated_on; echo date("d-M-Y", strtotime($updated_on)); ?></td>
                                        <td class="text-success"><b> <?php if ($row->subject_status == 1) {
                                                    echo "<b style='color:green'>Active</b>";
                                                } elseif($row->subject_status == 0) {
                                                    echo "<b style='color:red'>Inactive</b>";
                                                } ?>   </b></td>
   <td><input  style="color:black;width:50px;margin:5px;border-radius:5px;padding:0px 5px 0px 5px" type="text" name="priority[]"  value="<?php echo $row->subject_priority;?>" style="width:50px" class="number-class" /><span class="hide"></span></td>                                               
                                        <td> 
                                            <a href="<?php echo base_url(); ?>superadmin/Subjects/getSubjects/<?php echo $row->subject_id; ?>" class="btn btn-sm btn-warning "> Update <i class="glyphicon glyphicon-upload"> </i> </a><br/><br/>
                                        </td>
                                    </tr> 
                                    <?php $i++;
                                }
      ...
      // alert(listarray);
        ...
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'subjects', 'updatelist': checklist, 'activity': s},
                url: '<?php echo base_url(); ?>superadmin/Subjects/commonStatus/',
                success: function (u) {
                    console.log(u);
    ...
    var listarray=new Array();
      ...
      if(checklist!=''){
         ...
         $.ajax({
          dataType:'json',
          type:'post',
          data:{'tablename':'subjects','updatelist':checklist},
          url:'<?php echo base_url();?>superadmin/Subjects/commonDelete/',
          success:function(u){
           // console.log(u);
            if(u.code=='200'){$('#success').show();$('#success').html(u.description).addClass('alert alert-success');setTimeout(function() {window.location=location.href;},2000);}
                    if(u.code=='204' || u.code=='301' || u.code=='422'){$('#fail').show();$('#failmessage').html(u.description).addClass('alert alert-danger');}
       ...
       $('#fail').show();$('#failmessage').html('*  Please Select ').addClass('alert alert-danger');
        ...
        if(con == true) {
            $('.multi_select').prop('checked', true);  $('[name="multiple[]"]').prop('checked', true);  
            $('#priority').submit();
        }else{  }
    });
</script>