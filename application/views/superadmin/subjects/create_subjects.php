      ...
      <div class="row">
        <div class="col-md-12">
          <div class="box box-primary">
            ...
            <a href="<?php echo SUPER_ADMIN_FOLDER_PATH;?>Subjects/manageSubjects" class="btn btn-primary pull-right">Manage Subjects</a>
            </div>
                <!-- /.panel-heading -->
                <div class="panel-body">
                                ...
                                ?>
                  <?php echo form_open('superadmin/Subjects/addSubjects',array('id'=>'form_data')); ?>
                                        <div class="col-md-8 input_fields_wrap"> 
                                            <div class="navbar-form">
                                                <?php echo form_label('Subject Name','Subject Name')."<span style='col
                                                or:red' id='subject_name2'> * ".form_error('class_name')."</span> <br>";
                                    ...
                                    <div class="col-md-2 col-md-offset-1 "> 
                                             <div class="form-group">
                                                ...
                                                <input type="submit" name="btn_submit" value=" Submit " class="btn btn-md btn-success " id="btn_submit">
                                                 <a href="<?php echo base_url();?>superadmin/Subjects/manageSubjects" class="btn btn-md btn-primary " id="btn_back"> Back </a>
                                            </div>
                                        </div>     
                                    <?php echo form_close();?>
                               </div>
 ...
 $(document).ready(function () {
        ...
        $(add_button).click(function (e) { //on add input button click
            ...
            if (x < max_fields) { //max input box allowed
                ...
                $(wrapper).append('<div class="navbar-form mrtop"><input type="text" name="subject_name[]" class="form-control input_small class_form" placeholder="Enter Subject Name "/> <span class="unit_error" id="subject_error"></span> <a href="#" class="remove_field btn btn-sm btn-danger"><i class="glyphicon glyphicon-remove"></i></a></div><div class="clearfix"></div>'); //add input box
            }
        });
     ...
     $('#form_data').on('submit', function (i) {
       ...
       $('.class_form').each(function () {
        var sub_pattern=/^[a-zA-Z ]*$/;
        var sub=$(this).val();
            if (this.value == null || this.value == "") {
                $(this).css('border', '1px solid red');
                $(this).next('#subject_error').html('<b style="color:red;">Enter Subject name</b>');
                str = false;
            }
             else if(sub!="" && !sub_pattern.test(sub))
          {
               $(this).css('border', '1px solid red');
           $(this).next('#subject_error').html('<b style="color:red;">please enter valid data</b>');
            str = false;
          }
             else {
                $(this).css('border', '');
                 $(this).next('#subject_error').html('');
            }
        ...
        if(str == true){
            // data insert starts form hear 
             ...
             $.ajax({
                        ...
                        url: "<?php echo base_url(); ?>superadmin/Subjects/addSubjects",
                        contentType: false,
                        cache: false,
                        processData: false,
                            ...
                            switch (data.code)
                            {
                                ...
                                $('.success_msg').show();
                                    $('.success_msg').html(data.description).addClass('alert alert-success fade in');
                                      setTimeout(function () {
                                        window.location = "<?php echo base_url(); ?>superadmin/Subjects/manageSubjects";
                                    }, 3000);
                                    break;
                                case 204:
                                  $('.fail_msg').show();
                                    ...
                                    $('#btn_submit').show();
                                    setTimeout(function () {
                                        window.location = "<?php echo base_url(); ?>superadmin/Subjects/addSubjects";
                                    }, 3000);
                                case 301:
                                case 422:
                                case 575:
...
//                                            $('.form_loading_show').hide();
    ...
    function addMore(){
     ...
     append_data+='<input type="text" name="subject_name[]" value="" id="subject_name" maxlength="10" autocomplete="off" class="form-control input_small class_form" placeholder="Enter Subject Name">';
...
$('.multiple_append').on('click','.removeme',function() {
  $(this).parent().remove();
    ...
    // function removeMore(){
    //    var remove_data = '';
    //  remove_data+='<div class="col-md-8 input_fields_wrap">';
                                            
    //  remove_data+='<div class="navbar-form">';
    //  // append_data+='<label for="Subject Name">Subject Name</label>';
    //   // append_data+='<label for="Subject Name">Delete Name</label>';
    //  remove_data+='<input type="text" name="subject_name[]" value="" id="subject_name" maxlength="10" autocomplete="off" class="form-control input_small class_form" placeholder="Enter Subject Name">';
    // remove_data+='</div>';
    //   $('.multiple_append').remove(remove_data);
    // }
</script>
<!-- <script type="text/javascript">
...
<script type="text/javascript">
 function strict_subject() {
   var subject_name=$("#subject_name").val().trim();
    if(subject_name!='' && subject_name!=0){
      $.ajax({
        url:'<?php echo base_url(); ?>superadmin/Subjects/restrict_subject',
        method:'POST',
        dataType:'JSON',
        ...
        success:function(s){
          ...
          $("#subject_name-restrict").html(s.description);
          }
        },
        error:function(e){
          console.log(e);
...
<script type="text/javascript">
$(".class_form").change(function(){
  var sub_name=$(".class_form").val();
