      ...
      <div class="row">
        ...
        <div class="col-md-5 form-inline">
          ...
          <button  class="btn btn-success   btn-md" onclick="updateActivationStatus('1')"> Active </button>
           ...
           <div class="col-md-1">
           <a href="<?php echo SUPER_ADMIN_FOLDER_PATH; ?>language/add_language" class="btn btn-primary pull-right">Add New</a></div>
        </div>
                          ...
                          <?php
                            ...
                            if ($result->code == SUCCESS_CODE) {
                                foreach ($result->common_result as $row) {
                                    ?>
                                    <tr class="">
                                        <td><input type="checkbox" class="checkbox" name="multiple[]" value="<?php echo $row->lid;?>"> </td>
                                        <td><?php echo $i; ?> </td>
                                        ...
                                        <td class="text-success"><b> <?php if ($row->status == 1) {
                                             ...
                                             <td> 
                                            <a href="<?php echo base_url(); ?>superadmin/language/get_language/<?php echo $row->lid; ?>" class="btn btn-sm btn-warning "> Update <i class="glyphicon glyphicon-upload"> </i> </a><br/><br/>
                                        </td>
                                    </tr> 
                                    <?php $i++;
                                }
                            } else { ?>
                                <tr><td colspan="10" style="text-align:center; color:red;font-size:20px;">No class Found...</td></tr>
                            <?php }
                            ?>
      ...
      // alert(listarray);
        ...
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'language', 'updatelist': checklist, 'activity': s},
                url: '<?php echo base_url(); ?>superadmin/language/commonStatus/',
                success: function (u) {
                    console.log(u);
    ...
    var listarray=new Array();
      ...
      if(checklist!=''){
         ...
         $.ajax({
          dataType:'json',
          type:'post',
          data:{'tablename':'language','updatelist':checklist},
          url:'<?php echo base_url();?>superadmin/language/commonDelete/',
          success:function(u){
           // console.log(u);
            if(u.code=='200'){$('#success').show();$('#success').html("Data deleted successfully").addClass('alert alert-success');setTimeout(function() {window.location=location.href;},2000);}
                    if(u.code=='204' || u.code=='301' || u.code=='422'){$('#fail').show();$('#failmessage').html(u.description).addClass('alert alert-danger');}
                 },
          error:function(er){
            console.log(er);
