      ...
      <div class="col-md-12 col-sm-12 ">
        ...
        <div class="col-md-4 form-inline">
          ...
          <button  class="btn btn-success   btn-md" onclick="updateActivationStatus('1')"> Active </button>
           ...
           <a href="<?php echo SUPER_ADMIN_FOLDER_PATH; ?>ClassSubjects/createClassSub" class="btn btn-primary pull-right">Add New</a></div>
                        ...
                        <tbody>
                          <?php
                            ...
                            if ($data->code == SUCCESS_CODE) {
                                foreach ($data->subjects_list as $row) {
                                  
                                    ?>
                                    <tr class="">
                                        <td><input type="checkbox" class="checkbox" name="multiple[]" value="<?php echo $row->class_sub_id;?>"> </td>
                                        <td><?php echo $i; ?> </td>
...
<div class="modal fade" id="subjectModelShow" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
     <form action="" method="post" name="subject_form" id="subject_form" enctype="multipart/">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="exampleModalLabel" style="font-weight:bold;"><span id="disp_title"></span></h4>
      </div>
      <div class="modal-body">
          ...
          <input type="text" name="subject_id" id="subject_id" value="0">
      ...
      // alert(listarray);
        ...
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'class', 'updatelist': checklist, 'activity': s},
                url: '<?php echo base_url(); ?>superadmin/ClassSubjects/commonStatus/',
                success: function (u) {
                    console.log(u);
    ...
    var listarray=new Array();
      ...
      if(checklist!=''){
         ...
         $.ajax({
          dataType:'json',
          type:'post',
          data:{'tablename':'class','updatelist':checklist},
          url:'<?php echo base_url();?>superadmin/ClassSubjects/commonDelete/',
          success:function(u){
           // console.log(u);
            if(u.code=='200'){$('#success').show();$('#success').html(u.description).addClass('alert alert-success');setTimeout(function() {window.location=location.href;},2000);}
                    if(u.code=='204' || u.code=='301' || u.code=='422'){$('#fail').show();$('#failmessage').html(u.description).addClass('alert alert-danger');}
...
function upateSubjectModel(subid,subdata)
{
  $('#subject_name').val(subdata);
  $('#subject_id').val(subid);
  $('#disp_title').html('Subject : '+subdata);
  $('#subjectModelShow').modal('show');
}
...
$.ajax({
            type:'POST',
            dataType:'html',
            url:"<?php echo base_url(); ?>superadmin/ClassSubjects/updateClassSubject",
            data:{'subject_name':subject,'subject_id':subject_id},
