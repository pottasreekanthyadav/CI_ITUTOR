  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Manage Institute</li>
      </ol>
    </section>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              ...
              <div class="col-sm-12">
        <form method="post" action="<?php echo base_url();?>superadmin/Tuitioncenter_controller/search_institute" class="form-inline">
           <div class="col-sm-3">
                   <input type="text" class="form-control" name="loc_search" id="loc_search" placeholder="area or email or mobile or location">
              </div>
...
<option value="<?php echo $value->institute_type_id;?>"><?php echo $value->institute_type_name; ?></option>
         ...
         <div class="col-md-4">
           <a href="<?php echo SUPER_ADMIN_LINK; ?>Tuitioncenter_controller/add_institute" class="btn btn-primary   btn-md"> Add</a>
                          ...
                          <?php
                            ...
                            if (!empty($result)) {
                                foreach ($result as $row) {
                                    ?>
                                    <tr class="">
                                        <td><input type="checkbox" class="checkbox" name="multiple[]" value="<?php echo $row->institute_id;?>"> </td>
                                        <td><?php echo $i; ?> </td>
                                        <td><?php echo $row->institute_name; ?></td>
                                         <td><?php echo $row->newcate; ?></td>
                                          <td><?php echo $row->institute_email; ?></td>
                                          <td><?php echo $row->institute_mobile; ?></td>
                                        <td><?php echo $row->institute_city; ?></td>
                                        <td><?php echo $row->institute_location; ?></td>
                                         <td><?php echo $row->institute_timing; ?></td>
                                         <td><img  width="100" src="<?php echo base_url(); ?>uploads/institutelogo/<?php echo $row->institute_logo; ?>"></td>
                                          <?php 
                                         $string=$row->institute_about;
...
?>
                                      <td> <?php echo $string; ?><?php if(strlen($string)>50){ ?><a  data-toggle="modal" data-target="#myModal12" href="#" onclick="populate_more('<?php echo $row->institute_id; ?>')" class="text-primary bb"><b>Read More</b></a><?php } ?></td>
                                        <p style="display:none" id='mydescreption<?php echo $row->institute_id; ?>'><?php echo $row->institute_about; ?></p>

                                        <td class="text-success"><b> <?php if ($row->institute_status == 1) {
                                                    echo "<b style='color:green'>Active</b>";
                                                } elseif($row->institute_status == 0) {
                                                    echo "<b style='color:red'>Inactive</b>";
                                                } ?>   </b></td>
                                                <td><a href="<?php echo base_url("superadmin/Tuitioncenter_controller/edit_institute/$row->institute_id") ?>" class="btn btn-xs btn-warning ">Update<i class="glyphicon glyphicon-upload"> </i></a></td>
                               ...
                               <div class="modal-dialog modal-lg">
                                   <div class="modal-content">
                                       <div class="modal-header">
                                           ...
                                           <h4 class="modal-title" id="myModalLabel">Details About</h4>
                                       </div>
      ...
      // alert(listarray);
        ...
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'institute', 'updatelist': checklist, 'activity': s},
    ...
    var listarray=new Array();
      ...
      if(checklist!=''){
         ...
         $.ajax({'){$('#
          dataType:'json',
          type:'post',
          data:{'tablename':'institute','updatelist':checklist},
          url:'<?php //echo base_url();?>superadmin/Tuitioncenter_controller/commonDelete',
          success:function(u){
           // console.log(u);
            if(u.code=='200').show();$('#successmessage').html(u.description).addClass('alert alert-success');setTimeout(function() {window.location=location.href;},2000);}
                    if(u.code=='204' || u.code=='301' || u.code=='422'){$('#fail').show();$('#failmessage').html(u.description).addClass('alert alert-danger');}
                 },
          error:function(er){
            console.log(er);
