...
if ( ! function_exists('alpha_check'))
{
	function alpha_check($input){
		
		return $output=((preg_match('/^[a-zA-z]*$/', $input))) ? 1:0;
	}
}
/*Function For Checking  the value is integer or not end*/
if ( ! function_exists('alpha_specialchar_check'))
{
    function alpha_specialchar_check($input){
        
        return $output=((preg_match('/^[a-zA-Z0-9+.]*$/', $input))) ? 1:0;
    }
}
/*Function For Checking  the value is integer or not*/
if ( ! function_exists('alphaspace_check'))
{
	function alphaspace_check($input){
		
		return $output=((preg_match('/^[a-zA-Z][a-zA-Z ]*$/', $input))) ? 1:0;
	}
}
/*Function For Checking  the value is integer or not end*/
/*Function For Checking  the value is integer or not*/
...
{
	function address_check($input){
		
		return $output=((preg_match('/^[a-zA-Z0-9][a-zA-Z0-9,.()-\/ ]*$/', $input))) ? 1:0;
	}
}
/*Function For Checking  the value is integer or not end*/
/*Function For Checking  the address is valid or not*/
if ( ! function_exists('alphanumeric_check'))
{
    function alphanumeric_check($input){
        
        return $output=((preg_match('/^[a-zA-Z0-9][a-zA-Z0-9 ]*$/', $input))) ? 1:0;
    }
}

/*Function For Checking  the value is valid date or not*/
...
/*Function For Checking  the value is  valid date or not end*/
/*Function For Checking  the value is  date comparison*/
if ( ! function_exists('date_comparison'))
{
	function date_comparison($date){
        $timestamp = strtotime($date);
        if($timestamp)
        return $output=(($timestamp) <  date('Y-m-d')) ?    1 : 0;
        else
       return $timestamp ? $date : date('Y-m-d');
	}   
}
/*Function For Checking  the value is  date comparison end*/
/*Function For Checking  the value is skypid or not*/
if ( ! function_exists('skype_check'))
{
	function skype_check($input){
		
		return $output=((preg_match('/^[a-z0-9][a-z0-9\.,\-_]{5,31}$/i', $input)) || filter_var($input, FILTER_VALIDATE_EMAIL)) ? 1:0;
	}
}
/*Function For Checking  the value is skypid or not end*/
/*Function For Checking  the value is integer or not*/
...
if ( ! function_exists('mobile_check'))
{
	function mobile_check($input){
		...
		return $output=(is_numeric($input) && (strlen($input) ==10) && (preg_match('/^[6-9]{1}[0-9]{9}$/', $input)) ) ? 1:0;
	}
}
